function Investigar_Lugar_Estudio() {
    // Función que permite investigar el Estudio
    // Si las pistas del ático no es 0 podrá hacer :
    if (estudio.pistas_investigador != 0) {
        // Pediremos el nombre de la carta para potencia
        ...
        for (i = 0; i < cartas_mano.length; i++) {                              // Buscaremos la carta en la mano                         
            if (cartas_mano[i].icono_intelecto >= 1) {                          // Si la carta tiene almenos un icono de intelecto
                if (carta_usuario_usar == cartas_mano[i].nombre) {              // imagen_carta = (cartas_mano[i].imagen)
                    carta_para_descartar = cartas_mano.indexOf(cartas_mano[i])  // Si la carta que escribimos tiene el mismo nombre que una carta de la mano
                    ...
                    anadir_carta_descarte_graficamente()                        // Eliminamos la imagen de la carta que usamos de la mano  
                ...
                if (carta_usuario_usar == cartas_mano[i].nombre) {                  // Si la carta que escribimos tiene el mismo nombre que una carta de la mano
                    carta_para_descartar = cartas_mano.indexOf(cartas_mano[i])      // imagen_carta = (cartas_mano[i].imagen) Por si acaso dentro de añadir carta
                    ...
                    anadir_carta_descarte_graficamente()                            //Eliminanos las cartas de la mano y pasa a la zona de descarte
